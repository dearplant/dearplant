...
from app.modules.user_management.domain.models.subscription import SubscriptionPlan, SubscriptionStatus, PaymentMethod
...
class SubscriptionRepositoryImpl(SubscriptionRepository):
    ...
    async def create_subscription(
        ...
        try:
            ...
            return {
                ...
                "payment_method": new_subscription.payment_method.value if new_subscription.payment_method else None,
    ...
    async def cancel_subscription(
        ...
        try:
            ...
            if immediate:
                update_values["subscription_end_date"] = datetime.utcnow()
    ...
    async def get_subscription_analytics(
        ...
        try:
            ...
            # Total subscriptions by plan type
            plan_query = select(
    ...
    # Additional repository methods based on interface requirements

    async def bulk_update_billing_cycle(
        self,
        subscription_ids: List[UUID],
        new_billing_cycle: int
    ) -> int:
        """
        ...
        Args:
            ...
            new_billing_cycle: New billing cycle in days
            
        Returns:
            int: Number of subscriptions updated
        """
        try:
            # Calculate new end dates based on billing cycle
            new_end_date = datetime.utcnow() + timedelta(days=new_billing_cycle)
    ...
    async def count_by_plan_type(self, plan_type: SubscriptionPlan) -> int:
        """
    ...
    async def create(self, subscription_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            plan_type = SubscriptionPlan(subscription_data.get("plan_type", "free"))
            payment_method = PaymentMethod(subscription_data["payment_method"]) if subscription_data.get("payment_method") else None
    ...
    async def downgrade_subscription(
        ...
        Args:
            subscription_id: Subscription identifier
            new_plan: New (lower) subscription plan
        ...
        try:
            ...
            # Adjust subscription end date for new plan
    ...
    async def get_active_subscriptions(
        ...
        Returns:
            List of active subscription data dicts
        """
        try:
            query = select(SubscriptionModel).where(
                and_(
    ...
    async def get_by_id(self, subscription_id: UUID) -> Optional[Dict[str, Any]]:
        """
        Get subscription by ID (alias for get_subscription_by_id).
        
        Args:
            subscription_id: Subscription identifier
            
        Returns:
            Optional subscription data dict or None if not found
        """
    ...
    async def get_by_user_id(self, user_id: UUID) -> Optional[Dict[str, Any]]:
        """
        Get subscription by user ID (alias for get_subscription_by_user_id).
        ...
        Returns:
            Optional subscription data dict or None if not found
        """
        return await self.get_subscription_by_user_id(user_id)
    ...
    async def get_expiring_trials(
        ...
        Returns:
            List of subscription data dicts with expiring trials
        """
        try:
            expiry_date = datetime.utcnow() + timedelta(days=days_ahead)
    ...
    async def get_revenue_metrics(
        ...
        try:
            ...
            # Monthly revenue calculation (approximate)
            ...
            # Approximate revenue calculation (you should use actual pricing from config)
    ...
    async def get_subscriptions_for_renewal(
        ...
        Returns:
            List of subscription data dicts due for renewal
        """
        try:
    ...
    async def get_trial_conversion_rate(
        ...
        Returns:
            Dict containing conversion rate data
        """
        try:
            if not start_date:
                start_date = datetime.utcnow() - timedelta(days=30)
    ...
    async def process_subscription_renewal(
        ...
        Args:
            subscription_id: Subscription identifier
            renewal_period_days: Number of days to renew for
        ...
        try:
            ...
            # Default renewal period: 30 days for monthly, 365 for yearly
    ...
    async def reactivate_subscription(
        ...
        """
        try:
            # Reactivate with default 30-day period
    ...
    async def update_payment_method(
        ...
        try:
            ...
            # Optionally update payment_provider_id if present
    ...
    async def upgrade_subscription(
        ...
        Args:
            subscription_id: Subscription identifier
            new_plan: New (higher) subscription plan
            payment_method: Payment method for new plan
        ...
        try:
            effective_date = datetime.utcnow()
            plan_enum = SubscriptionPlan(new_plan_type)
            # Determine payment_method from payment_data if present
            payment_method = None
