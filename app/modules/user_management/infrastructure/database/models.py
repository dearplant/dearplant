...
class UserModel(DatabaseBase):
    ...
    password_hash = Column(String(255), nullable=False, comment="Hashed password using bcrypt")

    # Timestamps (Core Doc 1.1)
    created_at = Column(
        DateTime(timezone=True), nullable=False, default=func.now(), comment="Account creation date"
    )
    ...
    reset_token = Column(String(255), nullable=True, comment="Password reset token (nullable)")
    reset_token_expires = Column(
        DateTime(timezone=True), nullable=True, comment="Reset token expiration"
    )

    # Security and protection (Core Doc 1.1)
    failed_login_attempts = Column(
        Integer, nullable=False, default=0, comment="Failed login attempt counter"
    )
    account_locked = Column(Boolean, nullable=False, default=False, comment="Account lock status")
    ...
    provider_id = Column(String(255), nullable=True, comment="External provider ID")
    status = Column(
        String(50),
        nullable=False,
        default="pending",
        comment="Current account status: pending, active, suspended, banned",
    )
    subscription_tier = Column(
        String(50),
        nullable=False,
        default="free",
        comment="Current subscription tier: free, premium_monthly, premium_yearly",
    )
    account_locked_at = Column(
        DateTime(timezone=True), nullable=True, comment="Account Locked Time"
    )
    ...
    subscription = relationship(
        "SubscriptionModel", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
...
class ProfileModel(DatabaseBase):
    """
    ...
    display_name = Column(String(100), nullable=False, comment="User's display name")
    profile_photo = Column(
        String(500), nullable=True, comment="Profile image URL in Supabase Storage"
    )
    bio = Column(Text, nullable=True, comment="User biography (max 500 chars)")

    # Location and weather integration (Core Doc 1.2)
    location = Column(String(255), nullable=True, comment="User's location for weather data")
    timezone = Column(String(50), nullable=True, comment="User's timezone")
    ...
    theme = Column(
        String(20), nullable=False, default="auto", comment="UI theme preference (light/dark/auto)"
    )

    # Notification settings (Core Doc 1.2)
    notification_enabled = Column(
        Boolean, nullable=False, default=True, comment="Global notification toggle"
    )

    # Timestamps (Core Doc 1.2)
    created_at = Column(
        DateTime(timezone=True), nullable=False, default=func.now(), comment="Profile creation date"
    )
    ...
    notification_preferences = Column(
        ...
        nullable=False,
        default=dict,
        comment="User's structured notification settings",
    )
...
class SubscriptionModel(DatabaseBase):
    """
    ...
    plan_type = Column(
        String(50), nullable=False, default="free", comment="free/premium_monthly/premium_yearly"
    )
    status = Column(
        String(20), nullable=False, default="active", comment="active/inactive/cancelled/expired"
    )

    # Free trial management (Core Doc 1.3 - 7 days specified)
    trial_active = Column(Boolean, nullable=False, default=True, comment="Free trial status")
    trial_start_date = Column(DateTime(timezone=True), nullable=True, comment="Trial start date")
    trial_end_date = Column(DateTime(timezone=True), nullable=True, comment="Trial end date")

    # Subscription periods (Core Doc 1.3)
    subscription_start_date = Column(
        DateTime(timezone=True), nullable=True, comment="Paid subscription start"
    )
    subscription_end_date = Column(
        DateTime(timezone=True), nullable=True, comment="Paid subscription end"
    )

    # Payment integration (Core Doc 1.3)
    payment_method = Column(String(20), nullable=True, comment="razorpay/stripe")
    auto_renew = Column(Boolean, nullable=False, default=True, comment="Auto-renewal status")
